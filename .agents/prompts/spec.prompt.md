...
10. When linking tickets in the spec or chat output, resolve the folder path per [AGENTS.md](../../../../context-engine/AGENTS.md#clickable-reference-policy)'s Clickable Reference Policy: never synthesize it from a UUID, a store root, or an example path, and run a follow-up ticket-api command for the authoritative path if the first response omits it.
...
12. Follow [AGENTS.md](../../../../context-engine/AGENTS.md#escalation-rules)'s escalation rule: if required details are still ambiguous after a focused search, ask one concise clarification rather than guessing.
